...
import { collection, onSnapshot, query, orderBy, doc, updateDoc, setDoc, getDoc, addDoc, arrayUnion, arrayRemove } from 'firebase/firestore';
...
export default function Community() {
    ...
    const toggleReaction = async (post) => {
        if (!auth.currentUser) return alert("Log in to react!");

        const uid = auth.currentUser.uid;
        const reactedBy = post.reactedBy || []; // array of user IDs who liked this
        const isLiking = !reactedBy.includes(uid); // if my ID isn't in the list → I'm liking
        const postRef = doc(db, 'posts', post.id);

        await updateDoc(postRef, {
            reactedBy: isLiking ? arrayUnion(uid) : arrayRemove(uid),
            reactions: isLiking ? (post.reactions || 0) + 1 : (post.reactions || 0) - 1,
        });

        // Notify the post author (not yourself)
        const me = auth.currentUser.displayName || auth.currentUser.email;
        if (isLiking && post.author !== me) {
            await addDoc(collection(db, 'alerts'), {
                recipient: post.author,
                sender: me,
                type: 'like',
                message: `liked your project: "${post.projectName}"`,
    ...
    const handleFollow = async (authorName) => {
        ...
        // Only notify if we just FOLLOWED (not unfollowed)
        if (!isFollowing) {
            const me = auth.currentUser.displayName || auth.currentUser.email;
            await addDoc(collection(db, 'alerts'), {
                                        ...
                                        <div style={{ display: 'flex', gap: '1rem' }}>
                                            <button
                                                ...
                                                style={{
                                                    ...
                                                    background: 'none',
                                                    border: 'none',
                                                    color: post.reactedBy?.includes(auth.currentUser?.uid) ? '#00ffc8' : 'gray',
                                                    fontWeight: post.reactedBy?.includes(auth.currentUser?.uid) ? 'bold' : 'normal'
